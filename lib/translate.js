 ...
 */
export async function translateToChinese(text, retries = 3) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      const url = `${MYMEMORY_API}?q=${encodeURIComponent(text)}&langpair=en|zh-CN`;
      console.log(`[Translate] Translating (attempt ${attempt}):`, text.substring(0, 50) + '...');
      ...
      console.error(`[Translate] Error (attempt ${attempt}/${retries}):`, error);
      if (attempt < retries) {
